...
use sidra_seats::{
    accept_seat, invite_seat, materialize_founding, provision_seat, Capability, SeatFence, SeatId,
    SeatMemoryEngine, SeatRegistry, SeatWorkingMemory, SeatsConformanceSuite,
};
...
fn test_fence_no_self_widen_and_intersection_ac4() {
    ...
    founding_fence
        .capabilities
        .insert(Capability::parse("fs.read:vault/Sources/**").unwrap());
    ...
    use sidra_seats::ProvisionSeatArgs;

    let res = provision_seat(ProvisionSeatArgs {
        registry: &mut registry,
        seat_id: &second.id,
        requested_capabilities: illegal_caps,
        admitting_fence: &founding_fence,
        budget_ceiling_cents: 1000,
        firm_month_ceiling: 5000,
