...
use crate::lifecycle::{
    accept_seat, invite_seat, materialize_founding, provision_seat, ProvisionSeatArgs,
};
    ...
    pub fn verify_exit_criterion() -> Result<(), String> {
        ...
        let mut event_log: Vec<(u64, String, String)> = vec![
            (
                1,
                "principal".to_string(),
                "DirectiveCreated: System setup".to_string(),
            ),
            (
                2,
                "principal".to_string(),
                "EngagementCreated: Initial engagement".to_string(),
            ),
            (
                3,
                "agent.backend".to_string(),
                "TurnCompleted: Task 1".to_string(),
            ),
        ...
        let (_fence, _budget, _memory) = provision_seat(ProvisionSeatArgs {
            registry: &mut registry,
            seat_id: &second_seat.id,
            requested_capabilities: second_caps,
            admitting_fence: &founding_fence,
            budget_ceiling_cents: 50_000, // $500.00
            firm_month_ceiling: 100_000,  // $1000.00 firm ceiling
        ...
        event_log.push((
            5,
            "principal".to_string(),
            format!("SeatInvited: {}", second_seat.id.0),
        ));
        event_log.push((
            6,
            second_actor.clone(),
            "SeatAccepted: Sam Altman".to_string(),
        ));
        event_log.push((
            7,
            second_actor.clone(),
            "DirectiveCreated: Sam's Directive".to_string(),
        ));
