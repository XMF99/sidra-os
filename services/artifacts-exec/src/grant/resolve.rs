...
pub trait WorkOrderCapabilityResolver: Send + Sync {
    fn resolve_work_order_capabilities(&self, work_order_id: &str) -> Result<BTreeSet<Capability>, String>;
}
...
impl MockWorkOrderCapabilityResolver {
    ...
    pub fn set_work_order_grant(&mut self, work_order_id: impl Into<String>, grant: BTreeSet<Capability>) {
...
impl WorkOrderCapabilityResolver for MockWorkOrderCapabilityResolver {
    fn resolve_work_order_capabilities(&self, work_order_id: &str) -> Result<BTreeSet<Capability>, String> {
        self.grants
            .get(work_order_id)
            .cloned()
            .ok_or_else(|| format!("Producing Work Order '{}' not found or has no capability grant (ADR-0056)", work_order_id))
    }
}
