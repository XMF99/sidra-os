...
export const AiCscoAdvisorView: FC = () => {
  const { cscoRecommendations } = useSupplyChainStore();

  return (
    <Stack gap="20px" style={{ width: '100%' }}>
      <Heading level={3}>Virtual AI CSCO Advisor ⭐</Heading>
      <Text color="secondary">
        Intelligent virtual CSCO delivering real-time demand planning, supplier recommendations, inventory optimization, and procurement strategy.
      </Text>

      <Grid columns={2} gap="16px">
        {cscoRecommendations.map((rec) => (
          <Box
            key={rec.id}
            padding="20px"
