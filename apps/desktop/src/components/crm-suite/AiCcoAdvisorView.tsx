...
export const AiCcoAdvisorView: FC = () => {
  const { ccoRecommendations } = useCustomerIntelligenceStore();

  return (
    <Stack gap="20px" style={{ width: '100%' }}>
      <Heading level={3}>Virtual AI CCO Advisor ⭐</Heading>
      <Text color="secondary">
        Intelligent virtual CCO delivering real-time lead prioritization, next best action recommendations, churn risk prediction, and upsell advice.
      </Text>

      <Grid columns={2} gap="16px">
        {ccoRecommendations.map((rec) => (
          <Box
            key={rec.id}
            padding="20px"
