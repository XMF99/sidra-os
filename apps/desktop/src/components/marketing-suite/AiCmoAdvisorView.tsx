...
export const AiCmoAdvisorView: FC = () => {
  const { cmoRecommendations } = useMarketingGrowthStore();

  return (
    <Stack gap="20px" style={{ width: '100%' }}>
      <Heading level={3}>Virtual AI CMO Advisor ⭐</Heading>
      <Text color="secondary">
        Intelligent virtual CMO delivering real-time growth strategy, budget allocation recommendations, audience insights, and channel optimization.
      </Text>

      <Grid columns={2} gap="16px">
        {cmoRecommendations.map((rec) => (
          <Box
            key={rec.id}
            padding="20px"
